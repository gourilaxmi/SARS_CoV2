...
FIX (v2):
  - plot_reproduction_numbers() Panel D now uses log-scale x-axis so that
    workplace / school / community bars (small k) are visible alongside
    household (large k).
  - NaN k values (insufficient events) shown as hatched "no data" bars.
  - Panel B overdispersion-over-time y-axis also log-scaled.
  - Summary dashboard overdispersion panel updated to match.
"""
...
def plot_weekly_proportions(weekly_props: pd.DataFrame,
    ...
    ax.set_title("Weekly Proportion of Individuals with Plausible Infectors in Settings",
                 fontsize=13)
...
def plot_cluster_statistics(clusters: list[dict], rand_cluster_stats: list[dict],
    ...
    def get_stat(cluster_list, variant, stat):
        ...
        if stat == "n_clusters":
            ...
            return sum(c["size"] for c in vc) / max(
                sum(c["size"] for c in cluster_list), 1)
    ...
    for ax, (stat, label) in zip(axes, stats_labels):
        ...
        rand_vals  = [[get_stat(rc, v, stat) for rc in rand_cluster_stats]
                      for v in variants]
    ...
    fig.suptitle(
        "Transmission Cluster Statistics by Variant\n"
...
def plot_reproduction_numbers(weekly_Rc: pd.DataFrame,
    ...
    if not weekly_Rc.empty and "median_Rc" in weekly_Rc.columns:
        ...
        ax_a.plot(weeks_ind, weekly_Rc["median_Rc"],
                  label="Individual-based Rc (prioritised)",
                  color="#2196F3", lw=2)
    ...
    # ── Panel B: Overdispersion over time (log scale) ────────────────────────
    ax_b = fig.add_subplot(gs[0, 1])
    if not od_time.empty and "k" in od_time.columns:
        od_plot = od_time[od_time["k"].notna() & (od_time["k"] > 0)].copy()
        if not od_plot.empty:
            weeks_od = [str(w) for w in od_plot["week"]]
            ax_b.plot(weeks_od, od_plot["k"], color="#9C27B0", lw=2)
            ax_b.fill_between(weeks_od,
                               od_plot["k"] * 0.8,
                               od_plot["k"] * 1.2,
                               alpha=0.2, color="#9C27B0")
            # Log scale so small and large k values are both readable
            ax_b.set_yscale("log")
            ax_b.axhline(1.0, color="grey", linestyle="--", lw=1,
                         label="k = 1 (Poisson threshold)")
            ax_b.legend(fontsize=8)
    ax_b.set_title("B  Overdispersion (k) Over Time", fontsize=11)
    ax_b.set_ylabel("Dispersion parameter k\n(log scale, lower = more overdispersion)",
                    fontsize=9)
    ax_b.tick_params(axis="x", rotation=45, labelsize=7)
    ax_b.grid(alpha=0.2, which="both")

    # ── Panel C: Placeholder (weekly setting proportions are in Fig 1) ───────
    ax_c = fig.add_subplot(gs[1, 0])
    ax_c.set_title("C  Rc by Setting (Prioritised Trees)", fontsize=11)
    ax_c.text(0.5, 0.5,
              "Setting-stratified Rc\n(see weekly_setting_proportions)",
              ha="center", va="center", transform=ax_c.transAxes,
              fontsize=10, color="grey")
    ax_c.set_xlabel("Week")
    ax_c.grid(alpha=0.2)

    # ── Panel D: Overdispersion by setting — LOG SCALE ───────────────────────
    ax_d = fig.add_subplot(gs[1, 1])
    if not od_setting.empty:
        settings_od = od_setting[od_setting["setting"] != "total"].copy()
        settings_od = settings_od.sort_values("k", ascending=True,
                                               na_position="first")

        bar_colors  = [SETTING_COLORS.get(s, "grey") for s in settings_od["setting"]]
        k_vals      = settings_od["k"].values
        setting_lbs = settings_od["setting"].values

        # Replace NaN with a small placeholder for display
        k_display = np.where(np.isnan(k_vals), 1e-3, k_vals)

        bars = ax_d.barh(setting_lbs, k_display,
                         color=bar_colors, alpha=0.85)

        # Hatch NaN bars and annotate them
        for bar, k_val, s_name in zip(bars, k_vals, setting_lbs):
            if np.isnan(k_val):
                bar.set_hatch("///")
        ...
        # Log scale so household and small-k settings are both visible
        ax_d.set_xscale("log")
        ax_d.set_xlim(left=5e-4)
        ax_d.axvline(x=1, color="grey", linestyle=":", lw=1.5,
                     label="k = 1 threshold")
        ax_d.set_xlabel(
            "Dispersion k — log scale\n(k < 1 = superspreading, k > 1 = regular)",
            fontsize=8
        )
        ax_d.set_title("D  Overdispersion by Setting", fontsize=11)
        ax_d.legend(fontsize=8)
        ax_d.grid(axis="x", alpha=0.3, which="both")

        # Annotation box explaining the scale
        ax_d.text(
            0.98, 0.04,
            "Log scale used so all\nsettings are visible",
...
def plot_npi_effects(all_results: pd.DataFrame,
    ...
    ax_a = axes[0]
    total_res = all_results[
        (all_results["setting"] == "total") &
        (all_results["covariate"].isin(NPI_LABELS.keys()))
    ].copy()
    ...
    ax_b = axes[1]
    vacc_rows = all_results[
        (all_results["covariate"].isin(["vacc_one_dose", "vacc_two_doses"])) &
        (all_results["setting"] == "total")
    ].copy()
    age_rows = all_results[
        (all_results["covariate"].str.startswith("age_")) &
        (all_results["setting"] == "total")
    ].copy()
    ...
    combined = pd.concat([
        vacc_rows.assign(
            group="Vaccination",
            label=vacc_rows["covariate"].str.replace("vacc_", "").str.replace("_", " ")
        ),
        age_rows.assign(
            group="Age",
            label=age_rows["covariate"].str.replace("age_", "Age ")
        )
    ], ignore_index=True)
    ...
    plt.suptitle(
        "NPI Effectiveness and Individual Risk Factors\n"
...
def plot_summary_dashboard(weekly_props, od_setting, all_results, cluster_summary,
    ...
    ax1.set_title("① Weekly Setting-Linked Transmission Proportions",
    ...
    # ③ Overdispersion by setting — LOG SCALE (FIX)
    ax3 = fig.add_subplot(gs[1, 0])
    if not od_setting.empty and "k" in od_setting.columns:
        od_plot = od_setting[od_setting["setting"] != "total"].copy()
        od_plot = od_plot.sort_values("k", ascending=True, na_position="first")

        k_vals     = od_plot["k"].values
        k_display  = np.where(np.isnan(k_vals), 1e-3, k_vals)
        bar_colors = [SETTING_COLORS.get(s, "grey") for s in od_plot["setting"]]

        bars = ax3.barh(od_plot["setting"], k_display,
                        color=bar_colors, alpha=0.85)

        for bar, k_val in zip(bars, k_vals):
            if np.isnan(k_val):
                bar.set_hatch("///")
                bar.set_facecolor("lightgrey")
                bar.set_edgecolor("grey")
        ...
        ax3.set_xscale("log")
        ax3.set_xlim(left=5e-4)
        ax3.axvline(1, color="grey", linestyle=":", lw=1)
        ax3.set_xlabel("k  (log scale, ↓ more superspreading)", fontsize=9)
    ...
    if not all_results.empty:
        ...
        npi_res = all_results[
            (all_results["setting"] == "total") &
            (all_results["covariate"].isin(NPI_SHORT))
        ].copy()
    ...
    if not all_results.empty:
        ...
        if not vacc_res.empty:
            ...
            ax5.set_title("⑤ Vaccination Effect on Transmission",
                          fontsize=10, fontweight="bold")
            ax5.set_ylim(0, 1.3)
